...
One feature that differentiates `phylospatial` from similar packages is that in addition to working with binary presence/absence data, all functions also work with continuous quantities such as occurrence probabilities or abundances. These quantities are treated as weights throughout all the analyses in the package, avoiding the need to discard information by thresholding the data.
...
We can also look at patterns in the phylogenetic similarity among locations. The first step here is to compute the community phylogenetic distance between every pair of sites, using the `ps_dissim()` function to add a pairwise distance matrix as an additional component in our spatial phylogenetic data set. This function includes a range of scaling options and alternative dissimilarity indices; let's run an example using quantitative Sorensen's index (`method = "bray"`), weighting taxa by inverse range size (`endemism = TRUE`), and normalizing community sums to emphasize relative compositional differences (`normalize = TRUE`):
