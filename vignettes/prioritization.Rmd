...
Let's demonstrate that here; we'll use 1000 reps for expediency, but 10000 or more would be better for a real analysis:

```{r prob}
priority <- ps_prioritize(ps, init = init, cost = cost, n_reps = 1000,
...
priority <- terra::rast(lapply(lambda, function(x){
      ps_prioritize(ps, init = init, lambda = x, progress = FALSE)
}))
