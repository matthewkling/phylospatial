...
We can also qualitatively visualize compositional patterns by converting ordination axes to a set of colors representing how similar two sites are to each other, using the `ps_rgb()` function. Let's do that here, using the `"nmds"` (nonmetric multidimensional scaling) ordination algorithm, and then plot the result using `tmap::tm_rgb()`:
